...
def extract_cgpa(text):
    # Regular expressions to match CGPA/GPA patterns
    cgpa_patterns = [
        r'\b(?:CGPA|GPA)[:\s]*\b(\d+(\.\d+)?)\b',  # Match CGPA or GPA with associated keywords
        r'\b(\d+(\.\d+)?)\s*(?:CGPA|GPA)\b',  # Match numeric value followed by CGPA or GPA
        r'\b(\d+(\.\d+)?)\b'  # Match standalone numeric value
    ]
    ...
    for pattern in cgpa_patterns:
        ...
        # If matches are found, return the first one (you can modify this logic based on your use case)
        if matches:
            for match in matches:
                for group in match:
                    if group and group.strip():  # Check if the group is not an empty string or contains only whitespaces
                        cgpa_value = float(group)
                        return cgpa_value

    # If no matches found or no valid numeric value in the match, return None
    return None
